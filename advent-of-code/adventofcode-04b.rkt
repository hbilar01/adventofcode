...
; return a string with its letters sorted
; we use this to match palindromes up (the sorted characters in two
; palindromes will be the same)
(define (sort-string str)
  (list->string (sort (string->list str) char-ci<?) ))

; returns the number of times str occurs in lst
(define (count-strings-in-list str lst [count 0])
  (if (null? lst)
      count
      (if (equal? (sort-string str) (sort-string (car lst)))
          (count-strings-in-list str (cdr lst) (add1 count))
          (count-strings-in-list str (cdr lst) count))))

; return the number of times each string in lst occurs in lst
(define (count-all-strings-in-list lst orig-lst [count 0])
  (if (null? lst)
      count
...
(define (valid-phrase? lst)
  (= (length lst) (count-all-strings-in-list lst lst)))

; count valid pass phrases in a list of pass phrases
(define (count-valid-phrases lst)
  (if (null? lst)
      0
